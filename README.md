...
本体 ROM プログラムの boot シーケンス(Disk Unit からロードされる IRL 込み)によってはシグネチャ "AB" でも実行から除外されます。<br>
<br>
...
<br>
自作のユーザーROM を新たにデバッグしたい場合は src/emu.h の #define _DEBUG を有効(先頭 // の 2 文字を消す)にしてビルト&書き込みして下さい。<br>
Arduino IDE のシリアルモニタは「LFのみ」にして h 送信でデバッグコマンドヘルプが出てきます。<br>

![debug-help](/img/debug-help.jpg)

各種モニタ、ICE(in-circuit emulator) 等を使った事がある方には「あぁ」なコマンドだと思います。ソフィアシステムズ/SA710 に近づけようとしたけど、そこに労力入れても報われないので必要最低限しか用意していません(笑)
